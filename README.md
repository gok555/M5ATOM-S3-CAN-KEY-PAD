...
2026/02/13
## 🎙️ Ver 5.1 Update: 音声操作 & 温度警告機能

Web Speech APIを活用したハンズフリー操作と、エンジンの安全を守るモニタリング機能を追加しました。

### 1. ボイスコントロール (Voice Control)
画面上のマイクボタン「🎙️」をタップすることで、ボタン操作を声で行えます。ドライビング中の視線移動を減らし、安全にモード切替が可能です。

* **ON操作:** ボタンのラベル名（例：「スポーツモード」）を話すとONになります。
    * *誤操作防止:* 既にONの場合は「既にオンです」と音声フィードバックを行い、誤ってOFFにすることを防ぎます。
* **OFF操作:** 「オフ」「停止」「切って」などの言葉を含めて話すとOFFになります。（例：「スポーツモード、オフ」）
* **セキュリティ連携:** 生体認証ロック中のボタンは、音声操作でも解除できません。

### 2. 排気温度アラート (EGT Warning)
CANバスから取得した温度（排気温度等）に対し、アラーム閾値を設定できるようになりました。

* **設定:** Settings画面の `Alarm:°C` に温度（例: 950）を入力。
* **動作:** 設定温度を超過すると、以下の警告を行います。
    * **視覚:** 温度表示が**赤色（RED）**で点滅。
    * **聴覚:** 合成音声で「排気温度警告、〇〇度です」とアナウンス（15秒間隔のクールダウン付き）。

### ⚠️ 動作環境の注意
* **ブラウザ:** Google Chrome / Microsoft Edge (Chromium系) 推奨。
* **通信:** 音声認識エンジン（Google Cloud）を利用するため、インターネット接続が必要です。
* **権限:** 初回利用時にブラウザのマイク使用許可が必要です。
